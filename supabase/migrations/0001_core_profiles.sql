...
-- First create the organization tables if they don't exist
...
CREATE POLICY "Users can view active profiles"
  ...
  USING (
    is_active = true AND 
    (
      -- Users can see profiles in their same organization
      ...
      OR 
      -- Or if they don't have an organization, see all active profiles
      NOT EXISTS (
...
CREATE POLICY "Organization members can manage their organization"
  ON public.organizations
  FOR ALL USING (
    id IN (
...
DROP TRIGGER IF EXISTS handle_organization_roles_updated_at ON public.organization_roles;
...
DROP TRIGGER IF EXISTS handle_organization_invitations_updated_at ON public.organization_invitations;
