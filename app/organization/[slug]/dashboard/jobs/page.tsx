  ...
  const handleBookmark = async (jobId: string) => {
    try {
      ...
      if (res.ok) {
        ...
        // Refresh jobs to show updated bookmark status
        fetchJobs();
      }
    } catch (err) {
      toast.error("Failed to update bookmark node");
    }
  };
  ...
  return (
    <div className="container py-8 max-w-7xl mx-auto px-6 space-y-8 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        ...
        <Button 
          className="h-12 rounded-xl font-bold px-8 bg-indigo-600 hover:bg-indigo-700 shadow-lg shadow-indigo-500/10"
          onClick={() => router.push(`/organization/${slug}/dashboard/jobs/new`)}
        >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          ...
          { label: "Saved Points", value: jobs.filter(j => j.isBookmarked).length, icon: Target, color: "text-purple-600", bg: "bg-purple-50" },
        ].map((s, i) => (
          ...
          <Card key={job.id} className="border-none shadow-sm rounded-[2.5rem] bg-white/60 dark:bg-slate-900/60 backdrop-blur-xl overflow-hidden group hover:translate-y-[-4px] transition-all duration-300 border border-transparent hover:border-slate-50 dark:hover:border-slate-800 flex flex-col">
             ...
             <CardFooter className="px-8 pb-8 pt-0 flex gap-3">
                <Button 
                  ...
                  onClick={() => router.push(`/organization/${slug}/dashboard/jobs/${job.slug || job.id}`)}
                >
                   Apply Now
                </Button>
                <Button 
                  variant="ghost" 
                  size="icon" 
                  className={`h-11 w-11 rounded-2xl transition-all ${job.isBookmarked ? "bg-purple-100 text-purple-600" : "bg-slate-50 dark:bg-slate-800 text-slate-400 hover:bg-slate-100"}`}
                  onClick={() => handleBookmark(job.id)}
                >
                   <Briefcase className={`h-4.5 w-4.5 ${job.isBookmarked ? "fill-purple-600" : ""}`} />
                </Button>
             </CardFooter>
          </Card>
        ))}
