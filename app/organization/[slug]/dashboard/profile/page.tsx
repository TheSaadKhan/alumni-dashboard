    ...
    if (!profile) {
        return (
            <div className="container py-24 text-center space-y-8 max-w-sm mx-auto animate-in fade-in duration-700">
               ...
               <Button className="h-12 w-full rounded-2xl bg-indigo-600 hover:bg-indigo-700 shadow-xl font-bold uppercase tracking-widest text-[10px]" onClick={() => router.push("/auth/complete-profile/member")}>
                  Initialize Profile
               </Button>
            </div>
        );
            ...
            <Card className="border-none shadow-sm rounded-[3rem] bg-white/60 dark:bg-slate-900/60 backdrop-blur-xl overflow-hidden relative">
                <div className="h-48 bg-gradient-to-br from-indigo-700 via-indigo-800 to-indigo-950 relative overflow-hidden group">
                   ...
                   <div className="absolute top-8 right-8 z-10">
                      <Button className="bg-white/10 hover:bg-white/20 border-none text-white backdrop-blur-xl rounded-xl font-black uppercase text-[10px] tracking-widest h-10 px-6 shadow-xl" onClick={() => router.push(`/organization/${slug}/dashboard/profile/edit`)}>
                     ...
                     <CardContent className="p-8 space-y-6">
                        <div className="p-4 bg-slate-50 rounded-2xl border border-slate-50">
                           ...
                           <div className="flex items-center gap-2 mt-4 pt-4 border-t border-slate-100">
                              ...
                              <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Class of {profile.graduationYear || profile.graduation_year || "2024"}</span>
                           </div>
                        </div>
                     </CardContent>
                  </Card>
                        ...
                        {professional.company ? (
                           <div className="flex items-center justify-between p-6 rounded-[2rem] bg-slate-50/50 hover:bg-white transition-all border border-transparent hover:border-slate-50">
                              <div className="flex items-center gap-5">
                                 ...
                                 <div>
                                    <p className="text-base font-bold text-slate-900 uppercase italic leading-none">{professional.current_position || professional.currentTitle || "Elite Position"}</p>
                                    <p className="text-[9px] font-black text-blue-600 uppercase tracking-widest mt-1.5">{professional.company || professional.currentCompany || "Confidential Org"}</p>
                                 </div>
                              </div>
